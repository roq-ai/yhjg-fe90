...
import { getFees, deleteFeeById } from 'apiSdk/fees';
import { FeeInterface } from 'interfaces/fee';

type ColumnType = ColumnDef<FeeInterface, unknown>;

interface FeeListPageProps {
...
export function FeeListPage(props: FeeListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<FeeInterface>>(
    () => `/fees?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: FeeInterface) => {
    if (hasAccess('fee', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/fees/view/${row.id}`);
    }
  };
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/fees/view/${record.id}`} passHref legacyBehavior>
            <Button
          ...
          {hasAccess('fee', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/fees/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('fee', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="fee" />
        </Flex>

        {hasAccess('fee', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/fees/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
