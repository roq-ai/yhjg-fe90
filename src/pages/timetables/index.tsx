...
import { getTimetables, deleteTimetableById } from 'apiSdk/timetables';
import { TimetableInterface } from 'interfaces/timetable';

type ColumnType = ColumnDef<TimetableInterface, unknown>;
...
export function TimetableListPage(props: TimetableListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<TimetableInterface>>(
    () => `/timetables?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: TimetableInterface) => {
    if (hasAccess('timetable', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/timetables/view/${row.id}`);
    }
  };
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/timetables/view/${record.id}`} passHref legacyBehavior>
            <Button
          ...
          {hasAccess('timetable', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/timetables/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('timetable', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="timetable" />
        </Flex>

        {hasAccess('timetable', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/timetables/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
