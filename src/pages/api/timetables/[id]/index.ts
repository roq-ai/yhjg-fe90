...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getTimetableById() {
    const data = await prisma.timetable.findFirst(convertQueryToPrismaUtil(req.query, 'timetable'));
    return res.status(200).json(data);
  }

  async function updateTimetableById() {
    await timetableValidationSchema.validate(req.body);
  ...
  async function deleteTimetableById() {
    const data = await prisma.timetable.delete({
      where: { id: req.query.id as string },
    });
    return res.status(200).json(data);
  }
