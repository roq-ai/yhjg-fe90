...
import { getExaminations, deleteExaminationById } from 'apiSdk/examinations';
import { ExaminationInterface } from 'interfaces/examination';

type ColumnType = ColumnDef<ExaminationInterface, unknown>;

interface ExaminationListPageProps {
...
export function ExaminationListPage(props: ExaminationListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ExaminationInterface>>(
    () => `/examinations?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: ExaminationInterface) => {
    if (hasAccess('examination', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/examinations/view/${row.id}`);
    }
  };
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/examinations/view/${record.id}`} passHref legacyBehavior>
            <Button
          ...
          {hasAccess('examination', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/examinations/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('examination', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="examination" />
        </Flex>

        {hasAccess('examination', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/examinations/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
